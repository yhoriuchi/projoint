...
The reshaped data have attributes and levels that are sorted alphabetically. Often, howeer, you want to reorder the attributes and/or order the levels of a particular attribute. You may also prefer not to use the actual labels for attributes and levels used in your conjoint experiments; for example, for the purpose of presentation, you may want to make them shorter. This process has been a nightmare for most applied scholars using other packages. We make this process quite easy. You first save the labels using `save_labels()`. In the CSV file you save, you should add a column named `order` and specify the order of attributes and attribute-levels you want to display in your figure. You can also revise the labels for attributes and levels in any way you like. But *you should not make any change to the column named `attribute_id` and `level_id`. After saving the updated CSV file, you should use `read_labels()` to read it and save the object suiable for the next step (i.e., use `projoint()`).
...
You can compare the difference between the following two figures. The first figure is using the alphabetical order:
```{r, fig.height=8, fig.width=8}
projoint(out1, .estimand = "amce") %>% plot_projoint(.estimand = "amce")
```

The labels and order of all attribute-levels in the second figure is the same as the original figure (Figure 2) in [Mummolo and Nall (2017)](https://doi.org/10.1086/687569).
```{r, fig.height=8, fig.width=8}
projoint(out1_arranged, .estimand = "amce") %>% plot_projoint(.estimand = "amce")
```
