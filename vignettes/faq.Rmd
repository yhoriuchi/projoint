...
<details>
<summary>Conjoint designs originated in market research and psychology.</summary>

Historical uses of conjoint involves asking each respondent to rate each of two different profiles (e.g., of a product). Doing so meant that each rating could be thought of as a separate observation. This led researchers to arrange their data with one profile per row, and thus twice as many rows as respondents (i.e., "profile-level data"). However, when respondents are instead asked to choose between the two profiles, as has become common in political science, these profiles are dependent, as choosing one necessarily meant not choosing the other. While researchers often still stack their data in this way, they then need to correct the statistical problem the induced by arranging the data at the profile level. A much easier and preferable approach is to arrange your data at the level of the respondent's choice, so that each row in the data matrix includes information about both profiles together. Our AJPS article shows also that profile-level analyses limits the substantive questions you can ask of the data you already collected, again for no reason.
