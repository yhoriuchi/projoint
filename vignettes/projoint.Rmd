...
library(projoint)  # Ensure package functions are available
installed_file_path <- system.file("extdata", "mummolo_nall_replication_cleaned.csv", package = "projoint")
print(installed_file_path)  # Debugging: Print file path

if (!file.exists(installed_file_path)) {
  stop("ERROR: The file is missing inside R CMD check.")
} else {
  print("File exists! Proceeding with read_Qualtrics.")
}

dat <- projoint::read_Qualtrics(installed_file_path)  # Explicitly use projoint:: to ensure function is found
