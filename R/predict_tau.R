...
#' @import ggthemes
#' @import stats
#' @param .list A list generated by reshape_conjoint()
...
#'   .flipped = TRUE)
#'   
#' tau <- predict_tau(reshaped_data)
#' tau

predict_tau <- function(
    .list, # reshaped data: the second element of a list generated by reshape_conjoint
    .idvar = "id"
){
  ...
  task1 <- d2 %>% stats::setNames(str_c("task1_", names(d2)))
  task2 <- d2 %>% stats::setNames(str_c("task2_", names(d2)))
  ...
  g <- ggplot2::ggplot() +
    ...
    ggplot2::labs(y = "Percent agreement", 
                  ...
                  title = stringr::str_c(": Prediction = ", format(round(prediction, digits = 2), nsmall = 2)))
